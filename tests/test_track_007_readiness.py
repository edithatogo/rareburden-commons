...
def test_track_007_readiness_packet_is_fail_closed() -> None:
    ...
    assert packet["submission_readiness"]["status"] == "ready_for_owner_submission_but_not_registered"
...
def test_track_007_readiness_disables_unqualified_claims() -> None:
    ...
    assert {"comprehensive landscape", "global completeness", "independent novelty confirmation"} <= claims
