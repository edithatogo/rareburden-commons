...
def test_public_shift_followup_records_exact_remote_effects() -> None:
    ...
    assert payload["totals"] == {
        "private_references_removed": 182,
        "private_head_bytes_removed": 5_517_987_017,
        "private_lfs_objects_permanently_deleted": 138,
        "public_recovery_source_verified": True,
    }
    assert payload["repositories"]["private_source"]["head_bytes"] == 88_730_147_338
    ...
    assert payload["repositories"]["account_private_dataset_head_total"] == {
        "repositories": 2,
        "bytes": 88_868_072_373,
    }
    assert payload["hpo_public_frontier"]["public_release_objects"] == 288
    assert payload["hpo_public_frontier"]["eligible_matrix_objects"] == 288
    assert payload["hpo_public_frontier"]["remaining_matrix_objects"] == 0
    ...
    assert payload["hpo_public_frontier"]["eligible_matrix_complete"] is True
...
def test_public_shift_followup_preserves_rights_and_quota_boundaries() -> None:
    ...
    assert payload["boundaries"] == {
        ...
        "all_eligible_hpo_objects_archived": True,
    }
    assert len(payload["hpo_public_frontier"]["workflow_runs"]) == 25
    failures = {
