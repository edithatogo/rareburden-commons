"""Tests for Track 011 reference closeout script and authorization gates."""
...
from scripts.check_track011_reference_closeout import (
    DECISION,
    Track011CloseoutError,
...
def test_track011_authorization_passes_on_repository_root() -> None:
    validate_authorization(ROOT)
    validate_plan_and_registry(ROOT)


def test_track011_authorization_fails_closed_on_claim_drift(tmp_path: Path) -> None:
    shutil.copytree(ROOT / "docs", tmp_path / "docs")
    shutil.copytree(ROOT / "manifests", tmp_path / "manifests")
    shutil.copytree(ROOT / "results", tmp_path / "results")
    engine = tmp_path / "src/rareburden/demonstrator_bronchiectasis.py"
    engine.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(ROOT / "src/rareburden/demonstrator_bronchiectasis.py", engine)
    ...
    with pytest.raises(Track011CloseoutError, match="authorization scope drift"):
...
def test_track011_plan_requires_all_tasks_checked(tmp_path: Path) -> None:
    ...
    with pytest.raises(Track011CloseoutError, match="unchecked task"):
