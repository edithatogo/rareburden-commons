...
def test_atlas_release_status_matches_schema() -> None:
    gap_map = build_domain_gap_map(
        load_mapping(ROOT / "catalog/data_sources.yml"),
        load_mapping(ROOT / "examples/config/gap-map-needs.yml"),
    )
    ...
    notice = build_atlas_release_notice(
        ...
        reason="Synthetic withdrawal exercise.",
    )
    validate_instance(
        build_atlas_release_status(candidate, [notice]),
        load_mapping(ROOT / "schemas/atlas-release-status.schema.json"),
    )
