...
- [~] Pin exact Orphadata, UN WPP and WHO artifact URLs/releases and record hashes after the approved source pages expose stable download routes. Orphadata endpoints and hashes are recorded in `review.md` (2026-08-01); `docs/track-002-source-registration-template.md` records the required UN/WHO fields and safe manual-registration fallback; UN WPP and WHO remain open.
