...
def calculate_synthetic_component_summary(document: dict[str, Any]) -> dict[str, Any]:
    """Calculate bounded nonmonetary component summaries without valuation.

    Components remain separate by default.  Only complete, explicit values
    with the same unit and denominator can enter a declared perspective group;
    possible or unassessed overlap keeps a component out of aggregate groups.
    Monetary-shaped rows are reported as blocked rather than converted.
    """
    ...
    for component in components:
        ...
        if quantity["kind"] == "monetary_shaped":
            ...
            overlap = component["overlap"]["assessment_status"]
            if overlap == "assessed_no_overlap":
