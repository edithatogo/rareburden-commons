...
def combine(paths_and_raw: list[tuple[str, bytes]]) -> dict[str, Any]:
    ...
    return {
        ...
        "sources": sources,
        "resolutions": sorted(resolutions, key=lambda item: item["canonical_key"]),
    }
...
def main() -> int:
    ...
    result = combine([(str(path), path.read_bytes()) for path in args.inputs])
