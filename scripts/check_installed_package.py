...
def check_installed_wheel(wheel: Path, *, uv: str, python_version: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rareburden-installed-") as temporary:
        ...
        node_check = (
            ...
            "pd={'schema_version':'0.1.0','policy_id':'synthetic-policy',"
            ...
            "t=TemporaryDirectory();s=DurableNodePolicyStore(Path(t.name)/'policy.sqlite3');"
            "s.register_policy(pd,recorded_at='2026-08-01T00:00:00Z');"
            "s.register_query({'analysis_id':'installed-analysis','dimensions':['group'],"
            "'measure':'count'},overlap_group='installed-overlap',"
            "policy_id='synthetic-policy',recorded_at='2026-08-01T00:01:00Z');"
            "assert s.verify()==(1,1);s.close();t.cleanup()"
        )
