#!/usr/bin/env python3
"""Validate Track 021 reference closeout authorization and lifecycle state."""
...
DECISION = Path("docs/decisions/2026-09-06-track-021-owner-reference-disposition.yml")
PANEL = Path("docs/reviews/track-021-reference-output-panel-2026-09-06.yml")
REGISTRATION = Path("docs/track-021-rbc-g001-bounded-registration-2026-09-06.yml")
...
TRACK_ID = "021-external-governance-activation"


class Track021CloseoutError(ValueError):
    """The Track 021 closeout authorization or lifecycle state escaped scope."""
...
def _mapping(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise Track021CloseoutError(f"expected mapping in {path}")
    return value
...
def validate_authorization(root: Path) -> None:
    ...
    expected_claims = {
        ...
        "external_partnerships_confirmed": False,
        "external_authority_active": False,
    }
    if (
        decision.get("track_id") != TRACK_ID
        or decision.get("decision_type") != "bounded_governance_activation_reference_disposition"
        ...
        or authorization.get("scope") != "bounded external governance activation framework"
        or claims != expected_claims
    ):
        raise Track021CloseoutError("bounded owner authorization scope drift")

    # Verify panel review
    panel = _mapping(root / PANEL)
    if panel.get("track_id") != TRACK_ID:
        raise Track021CloseoutError("panel track binding drift")
    if panel.get("panel_assurance") != "simulated_role_separated_advisory_panel":
        raise Track021CloseoutError("panel simulation status drift")
    if panel.get("status") != "actual_output_review_passed":
        raise Track021CloseoutError("panel review status not passed")
    ...
    for artifact in (
        ...
        if not (root / artifact).exists():
            raise Track021CloseoutError(f"required artifact missing: {artifact}")
    ...
    for filename, expected_hash in EXPECTED_OUTPUT_HASHES.items():
        ...
        if not output_file.exists():
            raise Track021CloseoutError(f"output file missing: {output_file}")
        actual_hash = _sha256(output_file)
        if actual_hash != expected_hash:
            raise Track021CloseoutError(
    ...
    if (
        ...
        raise Track021CloseoutError("manifest metadata drift")
    runs = manifest.get("runs", [])
    if len(runs) != 2 or runs[0].get("exit_code") != 0 or runs[1].get("exit_code") != 0:
        raise Track021CloseoutError("manifest runs invalid or unverified")
...
def validate_plan_and_registry(root: Path) -> None:
    ...
    if re.search(r"^\s*-\s*\[ \]", plan_text, re.MULTILINE):
        raise Track021CloseoutError("Track 021 plan contains unchecked tasks")

    metadata = json.loads((root / METADATA).read_text(encoding="utf-8"))
    if metadata.get("status") != "complete":
        raise Track021CloseoutError("Track 021 metadata status is not complete")

    registry_text = (root / REGISTRY).read_text(encoding="utf-8")
    row_match = re.search(r"^\|\s*021\s*\|([^|]+)\|([^|]+)\|", registry_text, re.MULTILINE)
    if not row_match:
        raise Track021CloseoutError("Track 021 row missing from registry")
    status_cell = row_match.group(2).strip()
    if not status_cell.startswith("Complete"):
        raise Track021CloseoutError(f"Track 021 registry status {status_cell} must be Complete")


def main() -> None:
    parser = argparse.ArgumentParser(description="Check Track 021 reference closeout.")
    parser.add_argument("--root", type=Path, default=Path.cwd())
    args = parser.parse_args()

    validate_authorization(args.root)
    validate_plan_and_registry(args.root)
    print("Track 021 reference closeout validated successfully.")
