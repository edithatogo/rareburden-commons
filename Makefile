...
.PHONY: install sync validate validate-catalog validate-roadmap validate-landscape \
	...
	track-004-owner-rehearsal-check \
	...
	track-016-operations-audit-check \
	track-017-release-audit-check \
	track-021-reference-closeout-check \
	track-004-reference-closeout-check \
...
track-004-owner-rehearsal-check:
	PYTHONPATH=src:. $(PYTHON) scripts/check_track004_owner_rehearsal.py \
		--root .
...
track-013-reference-closeout-check:
	PYTHONPATH=src:. $(PYTHON) scripts/check_track013_reference_closeout.py \
		--root .

track-014-reference-closeout-check:
	PYTHONPATH=src:. $(PYTHON) scripts/check_track014_reference_closeout.py \
		--root .
...
track-016-operations-audit-check:
	PYTHONPATH=src:. $(PYTHON) scripts/check_track016_operations_audit.py --root .

track-017-release-audit-check:
	PYTHONPATH=src:. $(PYTHON) scripts/check_track017_release_audit.py --root .

track-021-reference-closeout-check:
	PYTHONPATH=src:. $(PYTHON) scripts/check_track021_reference_closeout.py \
		--root .

track-004-reference-closeout-check:
	PYTHONPATH=src:. $(PYTHON) scripts/check_track004_reference_closeout.py \
		--root .
