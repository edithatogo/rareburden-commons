...
- Aggregate-only synthetic cohort and offline runner with participant-field,
  nested-value, small-cell, replay and overlap-budget rejection.
- Deterministic wheel bundle and offline-install rehearsal with value-free
  receipts; current evidence is candidate/same-operator only.
- Correction/withdrawal lifecycle, immutable policy/query receipts and
  fail-closed policy comparisons.
...
Continue synthetic adversarial tests, wheelhouse portability and documentation.
Do not weaken custodian thresholds, connect to controlled data, claim a pilot,
or infer independent operation from local or hosted automation.
